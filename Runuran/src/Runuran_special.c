...
SEXP
Runuran_qpinv (SEXP sexp_unur, SEXP sexp_U)
     /*----------------------------------------------------------------------*/
     /* Evaluate approximate quantile function when a UNU.RAN object of      */
     /* type PINV is given.                                                  */
     /*----------------------------------------------------------------------*/
{
  ...
  if ( gen->method != UNUR_METH_PINV ) {
    error("[UNU.RAN - error] invalid UNU.RAN object: method 'PINV' required");
  }
  ...
  for (i=0; i<n; i++) {
    if (ISNAN(U[i]))
      ...
      NUMERIC_POINTER(sexp_res)[i] = unur_pinv_eval_approxinvcdf(gen,U[i]); 
  }
  UNPROTECT(1);
